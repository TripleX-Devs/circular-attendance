...
const PostAttendence = async (req: Request, res: Response, next: NextFunction) => {
    const AttendanceData: Attendance[] = req.body.data;

    if (!AttendanceData) return res.status(404).json({ message: "attendance data not received" });
    ...
    const AttendenceTableData: AttendenceTableData[] = AttendanceData.map(({ GroupName, ...rest }) => ({
    ...
    try {
        ...
        console.log(error)
        res.status(500).json({ message: "An error occurred while processing attendance data", error });
    }
};
